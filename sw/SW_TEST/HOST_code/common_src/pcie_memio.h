...
class fpga_dev_mem {
	...
	public :
		...
		int fpga_dev_mem_init(uint32_t PCIe_BASE_ADDR) {
                        if((fpga_fp_dev = open("/dev/mem", O_RDWR | O_SYNC)) == -1) FAUT_CONDITION;
                        fflush(stdout);
                        virt_page_base = mmap(NULL, (uint32_t)PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fpga_fp_dev, (PCIe_BASE_ADDR & ~(PAGE_SIZE-1)));
